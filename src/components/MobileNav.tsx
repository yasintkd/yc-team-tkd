...
const navItems = [
  ...
  { to: '/odemeler', label: 'Aidat', icon: CreditCard },
        ...
        {navItems.map(({ to, label, icon: Icon }) => (
          <li key={to} className="flex-1">
            <NavLink
              ...
              className={({ isActive }) =>
                [
                  'flex min-h-[52px] flex-col items-center justify-center gap-0.5 rounded-lg px-1 py-1.5 text-[10px] font-medium transition active:scale-95',
                  isActive
                    ? 'text-brand-cyan'
