          ...
          <div className={`w-full max-w-sm rounded-3xl p-8 text-center text-white shadow-2xl border-8 ${state.winner === 1 ? 'bg-blue-600 border-blue-400' : 'bg-red-600 border-red-400'}`}>
            ...
            {isAdmin && (
              <div className="mt-8 grid grid-cols-2 gap-3">
                <button onClick={(e) => { e.stopPropagation(); newMatch(true); }} className="rounded-2xl bg-white px-4 py-4 font-black text-slate-800 shadow-xl hover:bg-slate-100 text-xs">
                  AYNI SPORCULAR
                </button>
                <button onClick={(e) => { e.stopPropagation(); newMatch(false); }} className="rounded-2xl bg-slate-800 px-4 py-4 font-black text-white shadow-xl hover:bg-slate-900 text-xs">
                  YENİ MAÇ
                </button>
              </div>
            )}
